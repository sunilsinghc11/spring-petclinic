...
# `curl -v -G '${JF_RT_URL}/lifecycle/api/v2/promotion/records/${BUILD_NAME}/${BUILD_ID}?async=false' -H 'Content-Type:  application/json' -H 'Authorization: Bearer ${JF_ACCESS_TOKEN}'` > RB2_STATUS.txt

# export RB2_STATUS=`cat RB2_STATUS.txt`
# export RB2_STATUS=$(echo $RB2_STATUS | jq -r '.promotions[0].environment')
echo "JF_ACCESS_TOKEN = ${JF_ACCESS_TOKEN}"

export RB2_STATUS_RESP=$(curl -v -G https://psazuse.jfrog.io/lifecycle/api/v2/promotion/records/${BUILD_NAME}/${BUILD_ID}?async=false -H 'Content-Type:  application/json' -H "Authorization: Bearer ${JF_ACCESS_TOKEN}")

echo $RB2_STATUS_RESP > RB2_STATUS.txt

export RB2_STATUS=$(echo $RB2_STATUS_RESP | jq -r '.promotions[0].environment')
echo " Release Bundle state =  $RB2_STATUS "

# rm -rf RB2_STATUS.txt
